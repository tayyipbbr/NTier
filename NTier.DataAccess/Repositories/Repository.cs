...
namespace NTier.DataAccess.Repositories
{
        ...
        where T : class
    {
        ...
        {
           return await _context.Set<T>().Where(expression).FirstOrDefaultAsync(cancellationToken); // not alındı.

        }

        public IQueryable<T> GetWhere(Expression<Func<T, bool>> expression)
        {
            return _context.Set<T>().AsNoTracking().Where(expression).AsQueryable(); // asnoTracking listlerde gerek yok, düşür perfor artar.
        }
